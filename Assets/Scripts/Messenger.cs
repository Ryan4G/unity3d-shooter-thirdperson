...
public class Messenger<T, V>
{
    protected static Dictionary<string, List<Action<T, V>>> eventsDic = new Dictionary<string, List<Action<T, V>>>();

    public static void AddListener(string gameEvent, Action<T, V> action)
    {
        if (!eventsDic.ContainsKey(gameEvent))
        {
            List<Action<T, V>> actions = new List<Action<T, V>>();
            actions.Add(action);
            eventsDic.Add(gameEvent, actions);
        }
        else
        {
            List<Action<T, V>> actions = eventsDic[gameEvent];
    ...
    public static void RemoveListener(string gameEvent, Action<T, V> action)
    {
        if (eventsDic.ContainsKey(gameEvent))
        {
            List<Action<T, V>> actions = eventsDic[gameEvent];
    ...
    public static void Boardcast(string gameEvent, T t_value, V v_value)
    {
        if (eventsDic.ContainsKey(gameEvent))
        {
            List<Action<T, V>> actions = eventsDic[gameEvent];

            foreach (Action<T, V> a in actions)
            {
                a.Invoke(t_value, v_value);
            }
        }
    }
}
